...
function serverError(error) {
  return {
    statusCode: 500,
    headers: { "Content-Type": "application/json", ...corsHeaders() },
    body: JSON.stringify({ error: "server_error", details: String(error && error.message || error) })
  };
}
exports.handler = async function(event) {
  ...
  if (!process.env.DATABASE_URL) {
    return serverError(new Error("DATABASE_URL is not configured"));
  }
  let body = {};
  try {
  ...
  try {
    ...
    if (action === "getTasksForDate") {
      ...
      const res = await client.query("select * from tasks where date = $1 order by updated_at asc", [date]);
      return ok({ tasks: res.rows });
    }
    if (action === "getAllTasks") {
      const res = await client.query("select * from tasks order by updated_at asc");
      return ok({ tasks: res.rows });
    }
    if (action === "saveTask") {
      const { task } = body;
      if (!task || !task.id) return badRequest("task with id required");
      const params = [
        task.id,
        task.subject,
        task.type,
      ...
      await client.query(
        `insert into tasks (id, subject, type, content, lectures, duration, date, original_date, completed)
        ...
        `,
        [...params, task.updatedAt ? new Date(task.updatedAt) : null]
      );
      return ok({ ok: true });
    }
    if (action === "bulkSaveTasks") {
      const { records } = body;
      if (!Array.isArray(records)) return badRequest("records must be array");
    ...
    if (action === "moveTaskToDate") {
      const { taskId, newDate } = body;
      if (!taskId || !newDate) return badRequest("taskId and newDate required");
      await client.query("update tasks set date = $1, updated_at = now() where id = $2", [newDate, taskId]);
      return ok({ ok: true });
    }
    if (action === "bulkMoveTasks") {
      const { taskIds, newDate } = body;
      if (!Array.isArray(taskIds) || !taskIds.length || !newDate) return badRequest("taskIds[] and newDate required");
      await client.query("update tasks set date = $1, updated_at = now() where id = any($2::text[])", [newDate, taskIds]);
      return ok({ ok: true, moved: taskIds.length });
    }
    if (action === "bulkReassignDates") {
      const { records } = body;
      if (!Array.isArray(records) || records.length === 0) return badRequest("records[] required");
      const ids = [];
      const dates = [];
      ...
      if (ids.length === 0) return ok({ ok: true, moved: 0 });
      await client.query(
        `update tasks t
           set date = m.date, updated_at = now()
         from (
           select unnest($1::text[]) as id, unnest($2::text[]) as date
         ) as m
         where t.id = m.id`,
        [ids, dates]
      );
    ...
    if (action === "getAllProgress") {
      const res = await client.query("select * from progress order by updated_at asc");
      return ok({ progress: res.rows });
    }
    if (action === "getProgress") {
      const { subjectId } = body;
      if (!subjectId) return badRequest("subjectId required");
      const res = await client.query("select * from progress where id = $1", [subjectId]);
      return ok({ progress: res.rows[0] || null });
    }
    if (action === "saveProgress") {
      const { progress } = body;
      if (!progress || !progress.id) return badRequest("progress with id required");
      const params = [
        progress.id,
        progress.subjectId || progress.id,
        JSON.stringify(progress.completedTasks || []),
        progress.lastUpdated ? new Date(progress.lastUpdated) : null
      ];
      await client.query(
        `insert into progress (id, subject_id, completed_tasks, last_updated)
         values ($1,$2,$3,$4)
         on conflict (id) do update set
    ...
    if (action === "bulkSaveProgress") {
      ...
      for (const p of records) {
        const params = [
          p.id,
          p.subjectId || p.id,
          JSON.stringify(p.completedTasks || []),
    ...
    if (action === "getAllDailyChecks") {
      const res = await client.query("select * from daily_checks order by updated_at asc");
      return ok({ dailyChecks: res.rows });
    }
    if (action === "getDailyCheck") {
      const { id } = body;
      if (!id) return badRequest("id required");
      const res = await client.query("select * from daily_checks where id = $1", [id]);
      return ok({ dailyCheck: res.rows[0] || null });
    }
    if (action === "saveDailyCheck") {
      const { check } = body;
      if (!check || !check.id) return badRequest("check with id required");
      const params = [
        check.id,
        check.date,
        JSON.stringify(check.completedTasks || []),
        check.checkedAt ? new Date(check.checkedAt) : null
      ];
      await client.query(
        `insert into daily_checks (id, date, completed_tasks, checked_at)
         values ($1,$2,$3,$4)
         on conflict (id) do update set
           date = excluded.date,
           completed_tasks = excluded.completed_tasks,
    ...
    if (action === "deleteDailyCheck") {
      const { id } = body;
      if (!id) return badRequest("id required");
      await client.query("delete from daily_checks where id = $1", [id]);
      return ok({ ok: true });
    }
    if (action === "bulkSaveDailyChecks") {
      const { records } = body;
      if (!Array.isArray(records)) return badRequest("records must be array");
      for (const c of records) {
        const params = [
          c.id,
          c.date,
          JSON.stringify(c.completedTasks || []),
          c.checkedAt ? new Date(c.checkedAt) : null
        ];
        await client.query(
          `insert into daily_checks (id, date, completed_tasks, checked_at)
           values ($1,$2,$3,$4)
